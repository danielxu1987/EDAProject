...
df <- df |>
  mutate(
    smoking.kills.more = (Smoking.rate > High.body.mass.index.rate)
)
...
for (v in catVars) {
  ...
  cat(sprintf("%6s, log likelihood of: %g\n", v, logPred))
...
aucNull <- calcAUC(pred.Null, dCal[,outcome])
cat(sprintf("AUC of null model in calibration is: %g\n", aucNull))
