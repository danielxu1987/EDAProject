...
In this section we would like to understand our predictions with local interpretable model-agnostic explanations (LIME).

```{r lime, echo=TRUE, warning=FALSE}

df_merged$class <- as.numeric(df_merged[outcome] == TRUE)
pos <- 1

set.seed(12345)
intrain <- runif(nrow(df_merged)) < 0.75

train <- df_merged[intrain, ]
test <- df_merged[!intrain, ]

cat("Training set size is ", dim(train), " and test set size is ", dim(test))

# variables holding truth
#responses <- colnames(df_merged) %in% c(outcome, "class")

features <- ori_sel_colnames <- gsub("^pred\\.", "", selVars) # colnames(df_merged)[!responses]
...
cases <- sample(nrow(test), size = 6)
print(test[cases, c("class","pred")])

cm <- confusion_matrix(test$class, test$pred)
print(cm)
# all feature vars
##       predicted
## actual FALSE TRUE
...
pm <- performanceMeasures(test$class, test$pred, 'Logit')
print(pm)
# all feature vars
##   model precision    recall        f1 dev.norm
## 1 model 0.9645777 0.9503356 0.9574037      Inf

# print(summary(model_logr))
