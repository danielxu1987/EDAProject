...
Nive Bayers model is useful for predictions with many input variables and categorical variables with many levels. Since our categorical variables only contain 2 values true/false, they don't really improve the performance. Try it again with Country and Year as extra categorical variables.
...
Note the AUC score is getting better. Now run NB model with all selected variables so far.
...
It has improved to become 0.6 which is better than the *total.population* alone, also better then the combination of numerical variables.

## Decision tree model
```{r decision_tree, echo=TRUE}
library(rpart)
dt <- rpart(formula = Year ~ ., data = df_merged)
library(rpart.plot)
rpart.plot(dt)
