...
Our hypothesis is that the higher percentage of most influential death causes the more likely death rate will be high. So we will also derived binary values from *Unsafe.sex*, *High.blood.pressure* and *Smoking* to see if these death contribution is over their median values or not. 

Another reason to make extra binary is because so far there are only years and country which can be converted to factors. We would like to combine a couple of extra categorical variables in our prediction. Variable *Code* is less self-explanatory than Country and therefore we will remove it. 
...
After check the log likelihood and deviance reduction, no categorical or binary variables are doing a great job. Let is check how they perform in the ROC curve.
...
We will place the selected variables in the decision tree model.
...
The AUC increases to over 0.81 across training, calibration and test data. This is the highest score so far, better than single variable and Naive Bayers.
