...
Three values are checked in all numerical variables.  *NA, zeros* and *negative numbers*. 
...
Currently there is no NA's in the dataset, neither are there any negative values. Zero may indicate that either no one died of that specific cause in a country in a specific year, or they were missing values. Although it is possible to treat zero's as missing values and replace them with e.g. median or mean, doing this will overwrite all the true zero's which are reflecting no death.  Therefore, in this exploration attempt I will keep zero's as-is.

```{r zeros, include=TRUE}
```
