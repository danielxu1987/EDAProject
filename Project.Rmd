...
We will first create a rate to count the percentage of each specific casualty cause. Then visualise the stats with boxplot to see the major players worldwide. Also country (entity) and year will be converted to factor.
...
It seems high blood pressure, smoking, high glucose rate and high BMI (obesity) are the top reasons to people's death. Now we would like to see when smoking kills more than obesity.  So we will first generate an outcome column *`r outcome`* which indicates if smoking kills more than obesity does (binary value). This variable holds the ground truth to check against the prediction later. The country and year will be the categorical variables to evaluate in the single variable selection process.
...
catVars <- vars[sapply(df[,vars], class) %in% c('character', 'factor')]
...
The log likelihood and deviance reduction do not look great in both country and year.  From this perspective they do not help much in predicting the result. Let is check how they perform in the ROC curve.
...
plot_roc(dCal$pred.Entity, dCal[,outcome], colour_id=3, overlaid=T) # Entity is green
```

Entity seems to help more than Year since it is always higher than the null model. Now we are going to use all numerical variables for prediction and see which one performs the best to predict `r outcome`.
...
vars <- c('Entity', 'Unsafe.water.source', 'Diet.low.in.nuts.and.seeds', 'Household.air.pollution.from.solid.fuels', 'No.access.to.handwashing.facility', 'Child.stunting', 'Iron.deficiency')
...
AFter inspecting the best performing variables a bit more, *Entity* still seems a good relevant feature variable. *Unsafe.water.source* and *Household.air.pollution.from.solid.fuels* barely predict better than 50% with a high standard deviation that makes its performance low. 

Strange prediction for Entity can be > 1.0??
...
fig1 <- ggplot(dCal) + geom_density(aes(x=pred.Entity, color=as.factor(Smoking.kills.more)))
