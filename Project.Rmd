...
df_gdp_popl <- merge(df_merged_gdp, popl_df, by.x = c("Code", "Year"), by.y = c("Country.Code", "Year"), all.x = TRUE)

df_gdp_popl <- df_gdp_popl[, c(1:3, ncol(df_gdp_popl)-1, ncol(df_gdp_popl), 4:(ncol(df_gdp_popl)-2))]

print(count_nas(df_gdp_popl))

nas_df_gdp_popl <- df_gdp_popl[is.na(df_gdp_popl$gdp.per.capita) &
  is.na(df_gdp_popl$total.population),]
print(nrow(nas_df_gdp_popl))
print(nas_df_gdp_popl[sample(nrow(nas_df_gdp_popl), 6), 1:7])
```

It appears that some islands and the total value of the whole world from the original death causes are also included which results in 150 NAs in both gdp and total population. This is only 2.4% of all dataset. We think it is ok to drop them.

```{r drop_nas, echo=TRUE}
df_gdp_popl <- setdiff(df_gdp_popl, nas_df_gdp_popl)

# Our new dataset looks like:
df_gdp_popl[sample(nrow(df_gdp_popl), 6), 1:6]
...
df_v2$Country <- as.factor(df_v2$Country)
df_v2$Year <- as.factor(df_v2$Year)

df_v2$total.mortality <- rowSums(df_v2[numericVars])

for (col in numericVars) {
    df_v2[paste0(col, ".rate")] <- df_v2[col] / df_v2['total.mortality']
}

rateVars <- grep("\\.rate$", names(df_v2), value = TRUE)

figRateAll <- ggplot(stack(df_v2[rateVars])) + geom_boxplot(mapping = aes(x = ind, y = values)) + labs(x ='', y='Weight in total mortality') + coord_flip() 

print(figRateAll)

# count the death rate of all causes with respect to the total population
df_v2$Death.rate <- (df_v2$total.mortality / df_v2$total.population)
```

### Single variable prediction
It seems high blood pressure, smoking, high glucose rate and high BMI (obesity) are the top reasons to people's death. Now we would like to see what makes the death rate of a country over the median of the whole world.  So we will first generate an outcome column which indicates if death rate is higher than the worldwide median. This variable holds the ground truth to check against the prediction later. 

We are also interested in if the higher *Unsafe.sex* and *Alochol.use* have strong correlation with the death rate. So we will also derived binary variables to indicate if their percentage in total mortality is higher than median or not. Code is less self-explanatory than Country so we will remove it. The country and year will be the categorical variables to evaluate in the single variable selection process. 
...
df_v2 <- df_v2 |>
  mutate(
    ...
    Alochol.over.median = (Alochol.use.rate > median(Alochol.use.rate))
)
...
Visualise the ROC curve for all 3 variables.
...
plot_roc(dCal$pred.total.population, dCal[,outcome], colour_id=3, overlaid=T) # green
plot_roc(dCal$pred.Unsafe.sanitation, dCal[,outcome], colour_id=4, overlaid=T) #  blue

```
*Country* still appears to be the best performer, with *Unsafe.water.source* being the second best and *Household.air.pollution.from.solid.fuels* being the last which predicts less than the null model around $FPR = 0.8$. This means whether or not smoking kills more people than obesity is a country specific issue. We will take these 3 feature variables into the multiple feature model to see if using more variables can do a better job. 
...
# #include Year also in NB model
# selVars <- c('pred.Year', selVars)
# 
# # proportion of positive instances (outcome == pos) in the training set.
# pPos <- sum(dTrain[,outcome]==pos)/length(dTrain[,outcome])
# 
# nBayes <- function(pPos, pf) {
#   # proportion of negative instances (outcome != pos)
#   pNeg <- 1 - pPos
