...
We would like to know mroe about the relationship of GDP per capita and death causes per country. So we are going to merge the dataset as provided from [World Bank](https://data.worldbank.org/indicator/NY.GDP.PCAP.CD)
...
gdp <- gdp[, c('Country.Name', years)]
gdp_df <- melt(gdp, id.vars = "Country.Name", variable.name = "Year", value.name = "gdp_per_capita")
gdp_df$Year <- as.numeric(as.character(gdp_df$Year))
df_merged_gdp <- merge(df, gdp_df, by.x = c("Country", "Year"), by.y = c("Country.Name", "Year"), all.x = TRUE)
