...
We would like to predict **`r outcome`**. In the pairwise scatterplot from previous section it was clear that several diet related causes have a stronger linear correlation. Therefore we would like to pick these variables in dietFeatureVars `r dietFeatureVars`

In the pre-processing stage, We will create a rate where it counts the percentage or weight of each specific variable, with respect to the total death count from all causes in that row. Then generate an outcome column *`r outcome``* which indicates if it contributes to the mortality weight more than its median. This variable holds the ground truth to check against the prediction later. Do the same for selected variables which generate new columns, indicating if these causes contribute more over their median weight or not. These will be the categorical variables to evaluate in the single variable selection process.
...
The next step is to iterate through the generated True/False diet related variables and see which one has the best prediction chance. Anything that is selected should help predict the result better than a null model. Therefore LogNull, the log likelihood of null model, is used as a selection criteria. Only variables that can create better deviance reduction than the null model are printed in the result. The same threshold holds for both numerical and categorical variables.
...
selCatVars <- c()
minDrop <- logNull  # may need to adjust this number
...
# now go through all the diet related numerical variables in the `numericVars` vector and perform the predictions. Again, the outputs are stored back into the data frame.
for (v in rateVars) {
  pi <- paste('pred.', v, sep='')
...
In the result, all diet related variables perform poorly as they only have 50% chance of getting True or False. That is not better than a random guessing. iI the meantime, three variables seem to be most helpful, although their AUC values are not high (0.8+) either: 
