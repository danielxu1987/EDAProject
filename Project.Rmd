...
It is clear that total population (green) covers a larger area. Let us check the double density plot before drawing conclusion on which numerical variable does the best.
...
Both `r selNumVars[1]` and `r selNumVars[2]` seem to give reasonable predictions, only that one would perform better than the other depending on the measurement. To be honest, the `r selNumVars[2]` was not expected to be helpful in our prediction in the beginning. Also, three binary variables derived from the big contributors to people's deaths, which we expected to be strongly correlated with our outcome variable, do not stand out in this selection process. 

Our interpretation is that the higher the population the higher death rate will likely be. This may reflect the fact that developing countries usually have larger population than highly developed countries, where standards of living is higher and people theretend to live longer. 

Next to that, safe sanitation seems to be crucial. This is more about hand washing, clean environment and hygiene, which are among the very basic needs for living. If these basics are not met, then many other things can go wrong which can put people's health and life at risk. 
...
We would like to pick diet related death causes for exploration in this section. As the total number of countries (>200) is unreadable when visualising hierarchical structure in this report, we will only choose 30 of them. More specifically, we would like to focus on year 2000 which is the turn of the century. We will start with 5 clusters.
