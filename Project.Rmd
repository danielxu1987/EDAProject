...
Currently there is no NA's in the dataset. Actually most variables have zero values as shown in summary. This indicates that either no one died of that specific cause in a country in a specific year, which can happen sometimes, or they were just missing values. Furthermore, there are 690 negative values which obviously do not make sense.
...
My guess is that these zero's are missing values. Therefore it is good to explicitly indicate it by converting them to NA's, as well as negative values.

```{r zeros, echo=TRUE}
df_has_nas <- df
num_vars <- names(df[, -c(1:3)])
df_has_nas[names(df_has_nas) %in% num_vars][df_has_nas[names(df_has_nas) %in% num_vars] <= 0] <- NA # only replace zeros in the numerical variables, excluding year, country and code
print(sum(is.na(df_has_nas)))
```

It is good to know which columns have missing values, as well as which columns are more impacted than others.
...
nacounts <- count_missing(df_has_nas)
hasNA = which(nacounts>0)
print(sort(nacounts[hasNA], decreasing = TRUE))
```
