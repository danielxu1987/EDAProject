...
This is the R Notebook for the project from Sifeng (Daniel) XU, student ID 24525844 for unit *CITS4009 Computation Data Analysis*. It demonstrates the process and findings of EDA and ML based on the **Countries and Death Causes** dataset. Link to the video demo of my shiny app: https://youtu.be/gEgNz4DDMIM. 
...
It appears that some islands and *World* (containing total number for the whole world) are also included which results in `r nacount` NA's when integrating with total population. This is only `r nacount/nrow(df_merged)*100`% of all dataset. Since we are only interested in studying individual countries it is alright to drop them.
...
Now that total population is merged and tidied up, we would like to know which death causes have large impact worldwide. Since the absolute number of mortality can vary largely depending on the population of a country, a better measurement is the percentage of each death cause, with respect to the total casualties in that row. For example, if the total number of deaths in a year for country X is 1000 where 10 people died from outdoor air pollution, then $outdoor.air.pollution.rate=0.01$
...
It seems high blood pressure, smoking, high glucose rate and high BMI (obesity) are the top reasons to people's death. Now we would like to see what causes high death rate to a country. The rate is high (True) if it is over the median rate worldwide. This binary variable holds the ground truth to check against the prediction later. 
...
text(x = 0.4, y = 0.8, label = paste('Unsafe.sex.over.median (red)', 'Smoking.over.median (green)', "and Blood.pressure.over.median (blue) are overlapping", sep='\n'))
```

Their AUC values are not higher than the null model. This is the confirmation to their poor performance in log likelihood and deviance reduction.
...
It is clear that total population covers a larger area. Let us check the double density plot before drawing conclusion on which numerical variable does the best.
...
In contrast, the peaks of `r selNumVars[2]` for True/False are only marginally separated. It is harder to see when which binary value will more likely to appear. Therefore in this round of comparison `r selNumVars[1]` seems a better choice.

### Conclusion single variable selection

Both `r selNumVars[1]` and `r selNumVars[2]` seem to give reasonable predictions, only that one would perform better than the other depending on the measurement. To be honest, the `r selNumVars[2]` was not expected to be helpful in our prediction in the beginning. Also, three binary variables derived from the big contributors to people's deaths, which we expected to be more helpful in predicting our outcome variable, do not stand out in this selection process. 

Our interpretation is that the higher the population the higher death rate will likely be. This may reflect the fact that developing countries usually have larger population than highly developed countries, where standards of living is higher and death rate is lower. 

Next to that, safe sanitation seems to be crucial. This is more about hand washing, clean environment and hygiene, which are among the very basic needs for living. If these basics are not met, then many other things can go wrong which can push up the death rate. 

In addition, according to [WHO stats on 2019](https://ourworldindata.org/grapher/mortality-rate-attributable-to-wash#:~:text=Death%20rate%20attributed%20to%20unsafe,people%20of%20a%20given%20population), the higher death rate by unsafe water, sanitation and lack of hygiene comes mostly from developing countries in Africa, Asia and South America. 

![](./images/WHO_WASH_2019.JPG)

We will take these selected feature variables into the next round, feed them to multi-variate models to see if using multiple rather than single features can do a better job. 

## Naive Bayers model

We will first put selected numeric variables in Naive Nayers model for prediction.
...
```
Just to review, in the single variable prediction `r selNumVars[1]` had AUC:

+ 0.533 for training
+ 0.571 for calibration 
+ 0.529 for test. 

`r selNumVars[2]` had AUC
...
We will place the same set of variables from Naive Bayers in the decision tree model using *rpart*. In our first attempt we do not consider *Country* and *Year*.
...
cat(' The AUC for training set is: ', calcAUC(predict(tmodel, newdata=dTrain), dTrain[,outcome]), '\n The AUC for calibration set is :', calcAUC(predict(tmodel, newdata=dCal), dCal[,outcome]), '\n The AUC for test set is: ', calcAUC(predict(tmodel, newdata=dTest), dTest[,outcome]))
...
Our interpretation is that the high death rate is also dependent on year and country. This makes sense when comparing livability across countries and across a period of time. Highly developed countries tend to offer better places for life. In the meantime, some developing countries were also in the transitional phase where improvements to their economy and general standard for living took place in the years between 1990 and 2019.

## Clustering

We would like to pick diet related death causes for exploration in this section. As the total number of countries (>200) is unreadable when visualising hierarchical structure in this report, we will only choose 25 of them. More specifically, we would like to focus on year 2000 which is the turn of the century. We will start with 5 clusters.
...
In this cluster dendrogram some countries within the same cluster are geographically closer to others. For example, in cluster 2 South Africa, Tanzania and Mali are all in Africa. In cluster 3, Denmark and Belgium are two European countries. In cluster 5, Serbia and Croatia are neighbors. In cluster 1, Tuvalu, Solomon Islands, Samoa and Vanuatu are all island countries from Oceania. 

Our interpretation is that countries from the same region share similar dietary habits, and therefore people tend to die from similar causes. This clustering does show some pattern related to geographical proximity, although we are not sure yet if $k=5$ is the best option. Now we will visualise the clusters with 2 principle components.  
...
The ideal clustering result is when all points within a cluster are very close to each other (small distance within a cluster), and large distance between different clusters. Notice group 1, 2 and 4 are quite separated from each other while 3 and 5 seems to be close or even intersecting each other. Probably $k=5$ is not the best option.
...
```{r sel_k, echo=TRUE, warning=FALSE}
...
Given 25 countries selected randomly, the CH index has the highest value when $k=8$, with local maximum observed at $k=2$. Let us plot the hierarchical clustering again with this $k=8$.
...
In this dendrogram the pattern is clearer than the previous one in that neighbour countries form separate clusters of their own. See Serbia - Croatia, Vietnam - Japan (both from Asian), Armenia - Russian - Kazakhstan and three islands in Oceania. 

Next we will compare this performance with kmeans clustering and see if the best k value changes.  
...
Both CH index and average silhouette width (ASW) show a local maximum when $k=2$ and a highest value when $k=8$. This is in line with the observation from the hierarchical clustering. We can see in the visual comparison that clusters are farer apart from each other when $k=8$ than when k has other values. Therefore in this round of kmeans clustering 8 is also the best k value.

### LIME evaluation 

In this section we would like to understand our predictions with local interpretable model-agnostic explanations (LIME).  Firstly, we will use the generalised linear model (glm) to predict our outcome `r outcome`. We will take all (numeric) variables in this section.

```{r use_glm, echo=TRUE, warning=FALSE}
...
print(dTest[cases, c("class","pred")])

cm <- confusion_matrix(dTest$class, dTest$pred)
print(cm)

print(performanceMeasures(dTest$class, dTest$pred, 'Logit Test'))
print(performanceMeasures(dCal$class, dCal$pred, 'Logit Calibration'))

cat(' AUC for training set in logit: ', calcAUC(predict(model_logr, newdata=dTrain), dTrain[,outcome]), '\n AUC of test set in logit: ', calcAUC(predict(model_logr, newdata=dTest), dTest[,outcome]), '\n AUC for calibration set in logit: ', calcAUC(predict(model_logr, newdata=dCal), dCal[,outcome]))
```
Notice that based on all numeric variables, glm gives AUC values of 0.89 - 0.90. This is much higher than the AUC values (around 0.81) of selected variables with Decision Tree Model. This is a confirmation that logistic regression model is the go-to classifier when predicting binary outcome, which is our case here. 

Now we are curious which numeric variable actually contributes the most to this higher performance. We will create an explainer with the help of LIME and XGBoost below.
...
explanation1 <- lime::explain(example[1,], explainer, n_labels = 1, n_features = 4)
print(plot_features(explanation1))

explanation2 <- lime::explain(example[2,], explainer, n_labels = 1, n_features = 4)
print(plot_features(explanation2))

explanation3 <- lime::explain(example[3,], explainer, n_labels = 1, n_features = 4)
print(plot_features(explanation3))

explanation4 <- lime::explain(example[4,], explainer, n_labels = 1, n_features = 4)
print(plot_features(explanation4))

```

The explanation is exciting! The observations with the best fitting score (the last two) show **total population** as the most valuable feature for our prediction, especially when country's population is large (greater than 27 million). When the population is less than 4 million this variable becomes less important (still within the top 4 features) or even play a contradictory role to our prediction. This confirms our conclusion from previous sections that the larger the population the higher mortality rate will possibly be.

On the other hand, low bone mineral density rate and low birth weight rate (how much percent the number of death from these causes cover in total mortality) seem suprisingly useful to our prediction. We do not see the unsafe sanitation back though. Perhaps dying from low mineral density and birth weight (requires the mothers to eat properly) indicate that the food security is not guaranteed. Since food is a bottom line for survival, if food supply is not stable many other problems will occur that cause higher death rates for the whole community.
