...
nas_df_gdp_popl <- df_gdp_popl[is.na(df_gdp_popl$gdp.per.capita) & is.na(df_gdp_popl$total.population),]
...
outcome <- c('Unsafe.sex.kills.more') # Smoking.kills.more, sodium.over.median
pos <- TRUE # positive value 

df <- df |>
  mutate(
    #Smoking.kills.more = (Smoking > High.body.mass.index)
    Unsafe.sex.kills.more = (Unsafe.sex > Alochol.use),
...
fig1 <- ggplot(dCal) + geom_density(aes(x=pred.No.access.to.handwashing.facility, color=as.factor(Unsafe.sex.kills.more)))

fig2 <- ggplot(dCal) + geom_density(aes(x=pred.Country, color=as.factor(Unsafe.sex.kills.more)))
...
```{r}
plot_roc(dCal$pred.No.access.to.handwashing.facility, dCal[,outcome], colour_id=7) #Water source is gold
