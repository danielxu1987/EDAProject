...
In this section we would like to understand our predictions with local interpretable model-agnostic explanations (LIME).  Firstly, we will use the generalised linear model (glm) to predict `r outcome`. Since we will use the result of glm for explanation, we would like to take all (numeric) variables to find the best performing one.
...
cat('AUC of dTrain in logit: ', calcAUC(predict(model_logr, newdata=dTrain), dTrain[,outcome]))
cat('AUC of dTest in logit: ', calcAUC(predict(model_logr, newdata=dTest), dTest[,outcome]))
cat('AUC of dCal in logit: ', calcAUC(predict(model_logr, newdata=dCal), dCal[,outcome]))
```
...
```
The explanation is exciting! The observations with the best fitting score (the last two) show total population as the most valuable feature variable for our prediction, especially when country's population is large (greater than 27 million). When the population is less than 4 million this variable becomes less important (still within the top 4) or even play a contradictory role to our prediction. This confirms our conclusion from previous sections that the larger the population the higher mortality rate will possibly be.

On the other hand, low bone mineral density rate and low birth weight rate (how much percent the number of death from these causes cover in total mortality) seem unexpectedly useful to our prediction. We do not see the unsafe sanitation back. Dying from low mineral density and birth weight (requires the mothers to eat properly) may indicate that the food security is not guaranteed. Since food is a basic need for living, lack of food due to famine, war or poverty will likely cause the death rate to increase in a country/region. 