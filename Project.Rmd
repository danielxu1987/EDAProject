...
# count_strange() <- function(data, strange){
#   if(strange == 'na'){
#     sapply(data, FUN=function(col) sum(is.na(col)))
#   }
#   else if (strange == 'null'){
#     sapply(data, FUN=function(col) sum(is.null(col)))
#   }
#   else if (strange == 'zero'){
#     sapply(data, FUN=function(col) sum(col == 0))
#   }
#   else if (strange == 'negative'){
#     sapply(data, FUN=function(col) sum(col < 0))
#   }
# }
# 
# strange_cols <- function(data, strange){
#   counts <- count_strange(data, strange)
#   hasStrange = which(counts > 0)
#   counts[hasStrange]
# }
...
df_num <- df[, -c(1:3)] # get only numerical variables
...
Currently there is no NA's in the dataset, neither are there any negative values. Zero may indicate that either no one died of that specific cause in a country in a specific year, or they were missing values. Although it is possible to treat zero's as missing values and replace them with e.g. median or mean, doing this will overwrite all the zero's which are truly reflecting zero mortality.  Therefore, in this exploration attempt we will keep zero's as-is.


### Classficatioin 

In the pairwise scatterplot it was clear that several diet related causes has linear relation, although the correction is strong for some regions than others. Therefore we would like to explore here from classification perspective which variable is the best to predict *Diet.high.in.sodium* which will be our target variable. 

The feature variables are *Diet.low.in.fruit* and *Diet.low.in.vegetables*. We will create a rate where it counts the percentage of death from that specific cause, with respect to the total death count from all causes in that row. The goal is to see when both feature variables are over their median rate, whether the target variable will also be over its own median.

```{r addCat, echo=TRUE}
df_v2 <- df_num
df_v2$total.mortality <- rowSums(df_v2)
...
The next step is to process the numerical variables. Iterate through them all and see which variable has the best prediction chance.

```{r single_var_compare, echo=TRUE}
